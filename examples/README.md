...
| **[fft128_2die/](fft128_2die/)** | A **128-point FFT that does not fit one chip** — split at a stage boundary across **two dies** (chip 0 = the period-64 octant fold, chip 1 = stages 1..6), joined by one complex crossing. The first multi-chip design where a single DSP block spans chips: `whole(x) == die1(die0(x))`, **200/200 samples bit-exact** through the real two-chip system. Its README documents the complex-sample pacing the multi-chip drive requires. | 2 (2 chips) | `.kyt` or `.grc` |
| **[fft128_2p2s/](fft128_2p2s/)** | The same 128-point FFT **retargeted onto the real 2P2S dev board** — die 0 on chain A's head, die 1 on its tail, joined by the board's own on-carrier series link, with chain B wired and idle. **200/200 samples bit-exact** and **DRC-clean against `dev2p2s.kdb` itself**. Its README answers "do the two dies run concurrently?" with the measured per-die event timeline, and documents why per-chip sim clocks are not a shared time base. | 2 on a 4-die board | `.kyt` or `.grc` |
