...
| **[foc_motor/](foc_motor/)** | **Field-oriented motor control** — the command half of a PMSM current loop on one array: two **PI controllers** (32-bit anti-windup integrators) → **CORDIC inverse Park** rotation by a streamed rotor angle → **SVPWM** min-max injection → three inverter duty cycles as one 3-word packet, **bit-exact** against a host golden. The point is the MEASUREMENT: **17.86 µs per control iteration = 56.0 kHz** equivalent loop rate (simulated timing), above the 10–40 kHz industrial FOC band. Also measured and stated plainly: the chain sustains exactly **one** iteration — a second wedges, and so does a saturated drive — so there is no steady-state throughput to quote; and the *measurement* half (Clarke + forward Park) does not fit alongside it, because the scarce resource is **rendezvous-arm corridors, not cells** (55 of 120 cells, still 2 nets short). Hand-placed: **open the `.kyt` directly, don't import**. | 4 + 3 relays | `.kyt` |
