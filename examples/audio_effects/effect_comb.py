...
# Title: Feedforward comb (placed audio effect)
# Author: Lattrex
# Description: A placed feedforward COMB (x - 0.3*delay5(x), a single-fire Subtract join). Golden: the IDENTICAL stock-GNU-Radio chain (delay, multiply_const_ff, sub_ff) within a DERIVED per-block bound.
...
class effect_comb(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "Feedforward comb (placed audio effect)", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("Feedforward comb (placed audio effect)")
        ...
        self.settings = Qt.QSettings("gnuradio/flowgraphs", "effect_comb")
        ...
        self.scope = qtgui.time_sink_f(
            burst_len, #size
            samp_rate, #samp_rate
            "feedforward comb (x - 0.3*delay5)", #name
            1, #number of inputs
            None # parent
        )
        ...
        self.comb_sub = kyttar.subtract(device_id="kyttar_0", num_inputs=2)
        self.comb_gain = kyttar.gain(device_id="kyttar_0", gain=0.3, block_name="")
        self.comb_delay = kyttar.delay(device_id="kyttar_0", delay=5)


        ##################################################
        # Connections
        ##################################################
        self.connect((self.comb_delay, 0), (self.comb_gain, 0))
        self.connect((self.comb_gain, 0), (self.comb_sub, 1))
        self.connect((self.comb_sub, 0), (self.fx_out, 0))
        self.connect((self.fx_in, 0), (self.comb_delay, 0))
        self.connect((self.fx_in, 0), (self.comb_sub, 0))
...
def main(top_block_cls=effect_comb, options=None):
