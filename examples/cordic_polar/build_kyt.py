...
"""Regenerate ``cordic_polar.kyt`` from ``cordic_polar.grc`` via the real
import → guided placement → auto-route → build flow. Verified END TO END by
``cordic_polar_demo.py`` / ``verification/tests/test_cordic_polar_example.py``."""
...
def main():
    ...
    from cordic_polar_demo import KYT_PATH, import_and_pnr
