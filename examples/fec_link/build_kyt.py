...
"""Regenerate ``fec_link.kyt`` from ``fec_link.grc`` via the real auto flow
(import → generic auto place-and-route → build check). Verified END TO END
(TX bits + TX CRC word + burst-corrected RX bytes, all exact) by
``fec_link_demo.py`` / ``verification/tests/test_fec_link_example.py``."""
...
def main():
    ...
    from fec_link_demo import KYT_PATH, import_and_pnr
