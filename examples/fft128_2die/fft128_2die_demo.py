...
# Author: Lattrex
# Description: FFT128 across TWO DIES: chip 0 runs stage 0, chip 1 runs stages 1..6, joined by the inter-chip crossing. Driven through the placeKYT multi-chip GNURadio server.
...
class fft128_2die_demo(gr.top_block, Qt.QWidget):

    def __init__(self):
        gr.top_block.__init__(self, "Kyttar FFT128 — a 128-point transform across two dies", catch_exceptions=True)
        Qt.QWidget.__init__(self)
        self.setWindowTitle("Kyttar FFT128 — a 128-point transform across two dies")
        ...
        self.settings = Qt.QSettings("gnuradio/flowgraphs", "fft128_2die_demo")
        ...
        self.kyt_src = kyttar.source(device_id="kyttar_0", port_name="x16_in", num_channels=1, server_host="127.0.0.1", server_port=server_port, complex_in=True, burst_len=burst_len, stream_id="fft", pipelined=False, schedule="interleaved", repeat=False, output_words="auto")
...
def main(top_block_cls=fft128_2die_demo, options=None):
