...
* the power stage's resting **face must be NORTH** at `(9, 1)`. `default_cells`
  ...
  `no bus path from source to the broker tap`. (The builder sets NORTH; the
  saved `.kyt` records the face after the router's `ccw` auto-orient, so it
  reads `face: west` there.)
...
| `fft_spectrum_unreverse.py` / `fft_spectrum_to_db.py` (and the `_32_` pair) | the two embedded display blocks (`unreverse`, `to_db`) as GRC extracts them from the `.grc` |

(`fft_spectrum.py` / `fft_spectrum_32.py` are regenerated by GRC on Run and are
git-ignored — a fresh clone will not have them until GRC generates them.)
