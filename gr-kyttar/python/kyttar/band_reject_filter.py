"""
Kyttar Band Reject Filter Block for GNURadio
...
It mirrors GNU Radio's GRC **Band Reject Filter** (a fir_filter_fff whose taps come
from firdes.band_reject): the user specifies the stop-band in DSP units and the
chip block designs the firdes windowed-sinc taps internally.
...
class band_reject_filter(_PassThrough):
    """
    Kyttar Band Reject Filter — drop-in for GNU Radio's Band Reject Filter
    (filter.fir_filter_fff + filter.firdes.band_reject).

    GR marker; the real DSP runs on the placeKYT-hosted chip.

    Parameters (mirroring GNU Radio's Band Reject Filter verbatim):
        device_id: ID of the kyttar.device to use
        gain: passband gain
        samp_rate: sample rate in Hz
        low_cutoff_freq: lower stop-band edge in Hz
        high_cutoff_freq: upper stop-band edge in Hz
    ...
    def __init__(
        ...
        super().__init__(name="Kyttar Band Reject Filter", n_in=1, n_out=1)
        ...
        self._window = window
        self._beta = beta
        self._advertise_grc_params(device_id, "BandRejectFilter", {
