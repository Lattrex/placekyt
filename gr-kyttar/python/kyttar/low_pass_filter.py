"""
Kyttar Low Pass Filter Block for GNURadio
...
It mirrors GNU Radio's GRC **Low Pass Filter** (a fir_filter_fff whose taps come
from firdes.low_pass): the user specifies the filter in DSP units and the chip
...
class low_pass_filter(_PassThrough):
    """
    Kyttar Low Pass Filter — drop-in for GNU Radio's Low Pass Filter
    (filter.fir_filter_fff + filter.firdes.low_pass).
    ...
    def __init__(
        ...
        super().__init__(name="Kyttar Low Pass Filter", n_in=1, n_out=1)
        ...
        self._window = window
        self._beta = beta
        self._advertise_grc_params(device_id, "LowPassFilter", {
