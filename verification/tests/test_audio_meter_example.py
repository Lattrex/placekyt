...
"""Audio-tail + S-meter example — WHOLE-CHAIN end-to-end gate (AGENTS.md §5b).

Two analog streams duplex on one array (the BPSK-modem demux machinery):
...
0.02 FS floor ≈ 0.066 dB) — never tuned to pass. The AGC golden runs with
max_gain=0.999, the chip block's Q15 regime, exactly as the per-block gate
drives agc_ff (uncapped GR gain exceeds 1.0 near zero-crossings and the
trajectories split for the whole loop transient).

Whole-chain proof for 8 analog blocks that previously had only per-block
gates: DCBlocker, AGC, BandRejectFilter, Squelch, Abs, MovingAverage, Nlog10
(+ the duplex two-stream tagging path itself).
"""
...
from audio_meter_demo import (  # noqa: E402
    ...
    NLOG10_DB_SCALE, SIG, TONE_ONSET, TRANSIENT_TRIM, _jp, _q15, _s16, _wr,
    gr_golden,
    import_and_pnr, run_streams)
...
def test_import_pnr_build_ok(built):
    ...
    # two ingress streams, two tagged egress nets
...
def test_both_streams_within_derived_bounds(built, golden):
    project, bres, cat, ct = built
    a_gold, m_gold = golden
...
def test_squelch_actually_closes(built, golden):
    ...
    project, bres, cat, ct = built
    a_gold, _ = golden
    a_chip, _m = run_streams(project, bres, SIG)
    assert a_gold[-1] == 0.0
...
def test_shipped_kyt_saturated_matches_per_sample():
    """The shipped .grc drives both streams at Full-speed (``pipelined:
    'yes'``), so the whole-chain proof must hold SATURATED (INV-19/21): both
    interleaved streams' whole bursts queued back-to-back with NO quiescence
    must recover BIT-EXACTLY what the per-sample drive recovers, on both the
    audio and the meter rail."""
    import simkyt
    ...
    bres = BuildEngine(cat, CHIP_YAML).build(project, {project.chip_type: ct})
    assert bres.ok, [str(e) for e in bres.errors[:3]]
    a_ps, m_ps = run_streams(project, bres, SIG)
    ...
    for c in project.connections:
        if (isinstance(c.target, ChipPortEndpoint)
                ...
                and getattr(c, "out_tag", None) is not None):
            tag_to_sid[c.out_tag] = ("meter" if "nlog10" in c.source.block
                                     else "audio")
...
def test_mutation_agc_reference_FAILS(built, golden):
    ...
    try:
        ...
        assert bres2.ok
        a_gold, _ = golden
        a_chip, _m = run_streams(project, bres2, SIG)
        assert _audio_worst(a_chip, a_gold) > AUDIO_TOL_LSB, \
