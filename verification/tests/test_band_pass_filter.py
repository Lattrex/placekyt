...
"""Verify BandPassFilter against GNU Radio's firdes.band_pass + fir_filter_fff.

BandPassFilter is GRC's **Band Pass Filter**: a convenience FIR whose taps come
from ``filter.firdes.band_pass(...)``. The Kyttar block reproduces firdes in pure
Python (the runtime has no GNU Radio) and runs the taps on the verified
FIRFilterBlock datapath. Same two-tier proof as the low/high-pass: the
Q15-quantized taps are bit-exact firdes (INV-16), and the on-chip output matches
GNU Radio fir_filter_fff fed firdes taps. Every gate is paired with a failing
mutation (INV-4). firdes band-pass taps are linear-phase symmetric ⇒ delay=0.

Run:
    KYTTAR_GR_PYTHON=/usr/bin/python3 QT_QPA_PLATFORM=offscreen \
      <venv>/python -m pytest verification/tests/test_band_pass_filter.py -x -q
"""
...
from gr_kyttar.placement.blocks.band_pass_filter_block import BandPassFilter  # noqa: E402
...
output_float = list(firdes.band_pass(gain, fs, lo, hi, tw, window, beta))
""",
...
def _gr_bandpass_filter(inputs_q15, gain, fs, lo, hi, tw, window="hamming", beta=6.76):
    return run_gnuradio_ref(
        input_q15=inputs_q15,
        gnuradio_script="""
from gnuradio import gr, filter as gr_filter, blocks
from gnuradio.filter import firdes
taps = firdes.band_pass(gain, fs, lo, hi, tw, window, beta)
tb = gr.top_block()
...
def test_bp_taps_match_firdes_float():
    blk = BandPassFilter("bp", gain=1.0, samp_rate=32000, low_cutoff_freq=4000,
...
def test_bp_taps_q15_exact_all_windows(window, cfg):
    """Q15-quantized taps are BIT-EXACT to firdes for EVERY window — the on-chip
    filter IS the firdes band-pass."""
    gain, fs, lo, hi, tw = cfg
    blk = BandPassFilter("bp", gain=gain, samp_rate=fs, low_cutoff_freq=lo,
...
def _verify_vs_gr(params, inputs):
    blk = BandPassFilter("c", **params)
    dut = run_block_dut("BandPassFilter", inputs, params=params, chip_yaml=CHIP_YAML)
    assert dut.ok, f"build/run failed: {dut.reason}"
    gr = _gr_bandpass_filter(
...
def test_bp_matches_gnuradio_default():
    ...
    print(f"\nBP default (N={blk.num_taps},S={blk._head_shift},cells={blk.cell_count}):",
...
def test_bp_param_sweep(params):
    inputs = _random_input(seed=hash(tuple(sorted(params.items()))) & 0xFFFF, n=120)
    blk, dut, res = _verify_vs_gr(params, inputs)
    print(f"\nBP {params} N={blk.num_taps} cells={blk.cell_count}:", res.summary())
    assert res.passed, res.summary()


def test_bp_bitexact_reference():
    params = dict(gain=1.0, samp_rate=32000, low_cutoff_freq=4000,
                  high_cutoff_freq=8000, transition_width=2000, window="hamming")
    blk = BandPassFilter("c", **params)
...
def _mutation_setup():
    ...
    dut = run_block_dut("BandPassFilter", inputs, params=params, chip_yaml=CHIP_YAML)
    assert dut.ok, dut.reason
    gr = _gr_bandpass_filter(inputs, 1.0, 32000, 4000, 8000, 2000)
    return blk, dut, gr


def test_bp_mutation_inverted_fails():
    ...
    res = compare_against_grc(broken, gr.floats, metric=Metric.AMPLITUDE, delay=0,
                              op_count=blk.num_taps, head_shift=blk._head_shift)
    assert not res.passed, "gate failed to catch an inverted band-pass output!"


def test_bp_mutation_wrong_band_fails():
    blk, dut, _ = _mutation_setup()
    inputs = _random_input(seed=11, n=120)
    gr_wrong = _gr_bandpass_filter(inputs, 1.0, 32000, 6000, 12000, 2000)  # shifted band
...
def test_bp_mutation_delay_offset_fails():
    blk, dut, gr = _mutation_setup()
...
def test_bp_empty_output_fails():
...
def test_emit_report():
    ...
    write_report("BandPassFilter", res, coverage={
