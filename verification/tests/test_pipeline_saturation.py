...
NEEDS_BESPOKE = {
    ...
    "SVPWMBlock": "2-FACE LOCK RENDEZVOUS with TWO INDEPENDENT upstream "
        "producers (v_alpha, v_beta) feeding a 6-cell datapath and a 3-word "
        "packet egress: a 'sample' is one word on each of two DISTINCT faces "
        "from two SEPARATE blocks, so no shared harness can drive it (they "
        "all inject one stream through one port landing). Its saturated "
        "behaviour is gated BESPOKE in verification/tests/test_svpwm.py: "
        "test_saturated_equals_per_sample enqueues each sample's TWO ARM "
        "WORDS back-to-back as raw queue_words (the producers race at the "
        "rendezvous — the hazard the LOCK exists to survive) over a long "
        "mixed run and asserts equality with the per-sample reference; "
        "test_saturated_drive_is_not_vacuous pins that the stimulus would "
        "SHOW a mis-pairing. The block carries the INV-46 Rule 3 N+1-stop "
        "rotation (got_beta locks to the internal forward face; `scale` "
        "re-points LOCK_FACE with a backward WRITE.CFG @N,3 — declared via "
        "UNLOCK_CFG_ADDR=3). The RESIDUAL whole-burst limit — two or more "
        "complete pairs queued before running wedge after the first packet "
        "(measured stop_reasons: Deadlock at depth 2/3, EventLimit at 5) — "
        "is the TMRVoter's release-rides-the-abutting-cell boundary and is "
        "pinned by test_known_limit_saturated_burst_depth_is_one.",
    "CordicRotateBlock": "THREE-FACE LOCK-ROTATION RENDEZVOUS (x, y, theta "
        "on three DISTINCT faces from three SEPARATE upstream blocks) feeding "
        "a 20-cell rotation-CORDIC serpentine — no shared harness can drive "
        "it (they all inject one stream through one port landing). Its "
        "saturated behaviour is gated BESPOKE in test_cordic_rotate.py::"
        "test_saturated_equals_per_sample: each triple's THREE ARM WORDS are "
        "enqueued back-to-back as raw queue_words (the three producers race "
        "at the rendezvous — the hazard the LOCK rotation exists to survive) "
        "and the stream must equal the per-sample result bit for bit. The "
        "whole-burst limit is the SAME face-budget wall TMRVoterBlock "
        "documents (INV-46: N+2 = 5 faces needed, 4 available, so the "
        "serialize-LOCK release rides the ONE abutting cell and the block "
        "sustains one triple in flight): two or more complete triples queued "
        "before running deadlock — measured (depth-2 whole-burst -> explicit "
        "Deadlock, zero packets) and pinned by "
        "test_known_limit_whole_burst_depth_is_one.",
